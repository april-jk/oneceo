...
import { AdminButton, AdminDetailShell, AdminStickyInspector, AdminTabs, CodePanel, DangerConfirmDialog, DiffDrawer, StatusBadge, getAdminActionIcon, getAdminModuleIcon } from './admin-ui';
                ...
                <>
                  ...
                  <article className="admin-detail-section osac-release-notes">
                    ...
                    <div className="signal-list">
                      <p>已上传：{formatDateTime(selectedRelease.uploadedAt)} · 大小 {formatBytes(selectedRelease.sizeBytes)} · SHA256 {shortSha(selectedRelease.sha256)}</p>
                      <p>{selectedRelease.publishedAt ? `已发布：${formatDateTime(selectedRelease.publishedAt)} · latest${selectedRelease.id === currentPublishedReleaseId ? '是' : '否'}` : '尚未发布'}</p>
                    </div>
                  </article>
