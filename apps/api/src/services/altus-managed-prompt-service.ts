...
export class AltusManagedPromptService {
  ...
  buildSystemPrompt(input: {
    ...
    return [
      ...
      '- The sandbox browser defaults are fixed by the platform: `ONECEO_PLAYWRIGHT_CDP_URL=http://127.0.0.1:9222`, `PLAYWRIGHT_BROWSERS_PATH=/opt/ms-playwright`, and the global Playwright module path is preconfigured for platform browser tools. Do not search for global Playwright paths, do not set NODE_PATH manually, do not run `npx playwright install`, and do not create ad-hoc screenshot scripts such as `screenshot-test.mjs` for visual evidence.',
