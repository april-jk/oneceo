...
6. 侧边栏内容梳理顺序调整为：普通项目 -> 最近会话 -> 全部任务入口 -> 自组织项目
7. 最近会话必须作为独立分组显式展示，不再隐含在项目列表尾部
8. 最近会话默认可见数量提升到 6 条，优先保证用户在不打开弹窗的情况下定位近期会话
9. 自组织项目分组默认收起，避免长期占用侧边栏垂直空间
10. `/projects` 页面必须复用与侧边栏一致的项目源：普通项目读取真实项目列表，自组织项目读取同一份前端预览数据，并以下方折叠分组形式展示，避免两处各自维护一套示例数据
11. 自组织项目预览只允许在 `VITE_RUNTIME_ENV=dev` 时显示；`staging` 与 `product` 环境必须与侧边栏一起隐藏该分组
12. `/projects` 页面中的普通项目必须默认以展开态展示项目下真实会话列表，用户进入页面后无需再次点击展开即可看到“每个项目有哪些会话”
13. `/projects` 页面中的普通项目与会话展示必须采用树状层级结构：项目节点可折叠，会话作为其子节点缩进展示，并保持项目默认展开
