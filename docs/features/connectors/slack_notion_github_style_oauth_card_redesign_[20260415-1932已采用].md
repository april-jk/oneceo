...
Slack 与 Notion 详情页统一采用与 GitHub 相同的极简主卡片信息层级：
...
- `DialogContent` 必须保留 `DialogTitle` / `DialogDescription`
- 为避免占用卡片首屏空间，应使用 `sr-only` 隐藏标题描述，而不是删除语义节点
- 关闭按钮应采用悬浮覆盖方式，避免额外占用顶部栏高度
- 主内容区需适当下移，避免与右上角悬浮关闭按钮形成视觉拥挤
...
- 未授权态错误提示条
- 底部授权说明卡片 / 平台使用指南卡片
- `Supabase` 的“显示详情 / 隐藏详情”切换入口
- `Supabase` 卡片中的 `MCP 详细信息` 内容块
...
- 已授权态若仍存在异常，可保留必要错误提示；未授权态不展示错误，避免干扰主卡片视觉。
- 主卡片在无额外配置区时应居中展示，优先保证“一眼看完整卡片”。
- `Supabase` 不再额外展示 `MCP 详细信息` 区块，详情页只保留主卡片与必要状态。
- `Supabase` 在移除附加信息后，主卡片也采用同样的居中展示规则。
- `Vercel` 已连接标签固定显示产品文案，不直接回显 `team_xxx` 这类原始团队标识。
