...
本方案借鉴 DeepWiki 中 `code-yeongyu/oh-my-openagent` 的先询问、再清单过关、再计划执行思想，但在 oneceo 内收敛为更轻量的产品交互：
...
2. 将澄清问题从“自由文本追问”升级为“AI 生成的选择卡片”，降低用户输入负担。
3. UI 风格符合 oneceo “任务塔台”：紧凑、可扫描、可复核，不做营销式表单或浮夸卡片。
4. 首期只落地 PPT 场景，但 schema 可复用于报告、商业计划书、复杂网站等后续复杂交付物。
...
5. 每张卡片必须有一个推荐选项。
6. 每个选项必须说明对最终产物的影响。
7. 已从用户原话明确得到的信息不得重复询问。
8. 所有选项使用业务语言，不暴露内部工具、模型或技术名。
9. 每张卡片允许 `其他` 和 `补充说明`，但不依赖用户长文本。
10. 4 张卡片结束后必须生成 confirmed brief，不允许继续追问。
...
如果 AI 生成失败或 schema 不合格，首期允许使用内置 PPT fallback card plan，仍然最多 4 张。
...
- 补充说明：36px 到 72px 高的紧凑输入，placeholder 由 AI 生成。
