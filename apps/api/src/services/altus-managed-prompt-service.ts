...
export class AltusManagedPromptService {
  ...
  buildSystemPrompt(input: {
    ...
    return [
      ...
      '- [DEBUG_TODO_ANCHOR] When the user explicitly asks to debug, test, verify, or diagnose any feature, system behavior, or bug, OR when you encounter a failure that requires systematic debugging (debug_open_page unreachable, build failure, runtime error, test failure, etc.), you MUST call `debug_todo_write` BEFORE any debugging action to establish the test plan. After the debug plan is written, you MAY continue using `todowrite` to track repair and verification progress. Do NOT use `todowrite` as a substitute for `debug_todo_write` when establishing the initial debug plan.',
      '- After calling `debug_todo_write`, IMMEDIATELY show the user the debug plan in your next message: list each test unit with its expected input, expected output, and verification method. Do not proceed with testing without first confirming the plan with the user or at least making it visible. Keep the summary concise but specific enough that the user understands what will be verified.',
      '- The `debug_todo_write` tool requires: triggerReason (why debugging started), debugDepth (interface_only / real_link / storage_verification / full_link), and items (specific test units). Each item must include: id, testUnit (e.g. "POST /api/auth/login" or "function ensureNekoDebug()"), unitType (interface/function/page_action/redis_key/db_table/external_dependency/document_check), expectedInput, expectedOutput, boundaryConditions, and verificationMethod.',
      '- Do not write vague test units like "test the chat feature". Each unit must be specific enough to verify a single endpoint, function, page action, Redis key, or DB table.',
      '- After writing the debug todo, execute tests unit by unit. After each unit completes, record the actual result and update the item status (pending -> in_progress -> passed/failed/skipped). If a unit fails, record the exact deviation between expected and actual output.',
      '- When using debug_open_page or browser_interact to verify a debug todo unit, pass the matching debugTodoItemId. If the latest debug_todo_write result provides activeItemId, use that id for the next browser action unless a more specific item id is required.',
      '- When debugging requires locating a specific feature or code point in the project, FIRST query project documentation before searching code. The required documentation order is: (1) AGENTS.md for global rules and service boundaries, (2) docs/AGENTS_GUIDE/agent-code-locations.md for code position quick reference, (3) docs/AGENTS_GUIDE/AGENT_CODE_MODIFICATION_GUIDE.md for modification guidelines, (4) the latest adopted design document [yyyymmdd-hhmm已采用] for the current topic, (5) DESIGN.md for UI/UX constraints. Only after reading the relevant documentation should you perform code searches.',
      '- The documentation query result will tell you which files to look at and what keywords to search for. Do not bypass this step with ad-hoc global searches.',
      '- Debug conclusion must categorize findings into four classes: (1) implemented and verified as designed, (2) implemented but inconsistent with design, (3) design requirement exists but implementation is missing, (4) currently unverifiable - needs additional environment or evidence. Do not conclude with a plain "test passed" without evidence chain.',
