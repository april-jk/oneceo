...
- 已启用数据库的主页面进一步收敛为数据库工作台结构：
  ...
  - 部署设置页的左右两列容器不再额外显示标题栏，避免导航标题与当前分区标题重复占位。
  - 部署设置页左侧分区名改作右侧内容锚点；右侧一次性展示所有设置分区，点击左侧分区时滚动到对应内容块，而不是只切单个局部视图。
  - 左侧表列表只显示表名，不再叠加“应用数据库”之类的来源标签，避免挤压名称可读宽度；hover 时可查看完整表名。
  - 顶部“列 N”按钮打开列选择浮层，用户可控制当前数据表显示哪些列；表头、表体和空态 colspan 必须跟随可见列同步变化。
  - 记录详情默认隐藏，只有用户主动点击某条记录时才展开右侧编辑栏。
  - 点击数据表空白区域后再次收起右侧详情栏；该行为同时适用于“查看详情”和“新增记录”两种模式，避免默认长期占用横向空间。
...
- 工作台上传测试改为浏览器直传当前 Railway Bucket，保留原始文件名，不再经由 OneCEO API 中转文件流；进度条按浏览器到 Bucket 的真实上传进度展示。
- 浏览器直传只在用户主动进入存储桶页面并触发上传时发生；OneCEO 后端只生成短时 presigned POST 上传目标，文件正文不经由平台转发。
- 若 Bucket 响应本身未开放浏览器可读 CORS 头，前端应把上传提交视为 opaque success，并回到平台状态接口轮询文件列表确认对象是否已出现，而不是直接按浏览器读不到响应体判失败。
- 页面结构统一为“顶部资源总览卡 + 下方双列工作区”：
  ...
  - 进一步参照数据库页收口后，已启用态优先展示“顶部轻工具栏 + 下方左右工作区”，移除冗余说明卡和占位提示文本，只保留真正可管理的连接信息、状态摘要和应用变量。
...
- Railway Bucket 的公开访问方式和删除对象能力需要在后续阶段继续通过 Railway API / S3 SDK 实测确认；对象列表已接入状态查询主链。
- 存储桶页面已支持浏览器直传当前 bucket 的单文件上传，且保留原始文件名；后续如需覆盖保护、批量上传、下载、删除确认和文件预览，应继续在同一条 Railway Bucket 主链上补齐。
- 存储桶页面已支持点击文件查看详情，并在详情区执行下载、删除等管理动作；点击外部空白区域时，详情区会收起。
- 部署设置页已改成左侧 sticky 锚点导航 + 右侧长页内容块；左侧激活态需跟随右侧滚动位置同步，避免在长屏下丢失当前分区。
- 部署设置页的导航容器和内容区块应保持透明底，不再额外套厚重卡片背景；视觉重点通过锚点标题、边框和激活态指示表达，而不是多层容器叠加。
- 设置页左侧锚点导航不再显示额外栏目标题，导航列宽保持更窄，优先把可视宽度让给右侧内容区。
- 设置页右侧滚动容器需要预留足够的顶部安全间距，锚点定位和当前分区判断都必须使用同一套较大 offset，避免标题被上方固定区域遮挡。
...
- 如果用户项目是纯前端静态应用，业务上传能力仍应由应用自己的后端或签名链路承接；当前浏览器直传只用于 OneCEO 工作台内的资源管理，不代表可把 Railway 存储写密钥直接下发给用户站点运行时代码。
