...
async function exec(cmd:string){const rs:any=await kvmConnector.execSession(sid,{path:'/bin/bash',args:['-lc',cmd],capture_output:true,timeout_seconds:120});const id=rs?.data?.jobId||rs?.data?.job_id;const fin:any=id?await waitJob(String(id)):rs;const d=fin?.data?.result||fin?.data||{};console.log('CMD',cmd);console.log('STDOUT',String(d.stdout||d.output||''));console.log('STDERR',String(d.stderr||''));}
await exec('id && whoami');
await exec('sudo -n true; echo sudo_ok=$?');
await exec('sudo -n pkill -f /opt/.altus/opencode/osac; echo pkill_rc=$?');
