...
2026-05-27 已落地第一阶段低风险实现：

1. 新增 `ppt-archetype-routing-service`，根据最近用户消息和 confirmed brief 路由 `Deck Archetype`。
2. 当前支持区分投资人路演、内部战略汇报、品牌业务推介、行业研究、技术研讨、产品方案、咨询建议和周期复盘。
3. `AltusManagedPromptService.buildRuntimeContextPrompt()` 只在当前任务是 PPT 且不处于澄清等待态时注入 `# PPT generation contract`。
4. 非 PPT 任务不注入该 contract，避免污染通用 Altus Agent。
5. 首批测试覆盖：
   - 投资人路演包含 `moat_stack` 与 `ask_or_use_of_funds`。
   - 内部战略汇报包含 `decision_options`，且不包含 `ask_or_use_of_funds`。
   - 企业官网等非 PPT 任务不产生 PPT context。

## 17. 资料覆盖门槛补充记录

2026-05-27 管理后台复查测试会话 `6a65b010-9dbf-4d56-ab32-389b268c424c` 后发现：最新 PPT run 已正确进入 `investor_pitch` 范式并完成渲染，但工具链只有 `web_search`，没有 `web_extract`；最终摘要和幻灯片中却使用了招股书、IDC、弗若斯特沙利文等高冲击数据来源表述，证据底座不够厚。

因此第一阶段补充一个 PPT 作用域内的 `Source coverage guidance`。它是质量偏好和降级规则，不是阻断式门控；资料偶然不足时不能中断 PPT 交付链路。

1. 每个 archetype 都声明建议资料覆盖桶，例如投资路演尽量覆盖公司披露、财务/IPO、产品技术验证、市场规模、竞争、风险、团队/融资。
2. 高冲击数字不能只来自 `web_search` 摘要，必须对关键来源调用 `web_extract`，否则要降级为定性表达。
3. 12-15 页投资路演优先要求至少 6 个不同可引用来源；资料不足时必须写入 `openQuestions`，不能用 unsupported figures 填空。
4. 用户在 confirmed brief 中选择的视觉/叙事风格必须保留到 deck name、final summary 和 style kit，不能被 archetype 默认风格覆盖。
5. 如果合理检索/解析后仍然资料不足，必须继续渲染交付，同时在来源说明、openQuestions 或最终摘要中说明缺口并降低结论强度。

该补充仍只在 `# PPT generation contract` 内生效，不改变非 PPT 任务链路。

## 18. 2026-05-27 HTML 中间形态修订说明

`20260527_PPT_HTML中间形态导出PPT方案_[20260527-1046已采用].md` 已采用，并把本方案中“不要求 HTML 作为 PPT 中间形态”的判断修订为：PPT 任务默认进入 PPT 专用 HTML Deck 分支。

该修订只影响 PPT scoped 生成链路，不改变本方案已确立的 Deck Archetype、Evidence-first、PPT task-scoped context 和非 PPT 隔离原则。非 PPT 任务仍不得被 PPT prompt、PPT renderer 或 HTML deck contract 污染。
