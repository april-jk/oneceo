...
stop_port_if_matches_repo() {
  ...
  while IFS= read -r pid; do
    ...
    if [[ "${cmd}" == *"${ROOT_DIR}"* ]]; then
      ...
      echo "[INFO] Port ${port} is used by non-oneceo process (pid=${pid}), skipped."
    fi
  done <<< "${pids}"
}
...
# Fallback: stop listeners on common dev ports when command belongs to this repo.
stop_port_if_matches_repo 4000 "API"
stop_port_if_matches_repo 3000 "Web"

echo "[DONE] stop-mac completed."
