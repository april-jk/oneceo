...
| M18 | `19_write_file过程原子消息放大交互设计_[20260409-0038已采用].md` | `write_file` 运行中将工具原子消息临时放大，固定宽高展示代码新增预览，结束后自动收起的 UIUX 设计方案（Issue #37 后续体验增强） |
| M19 | `20_网站交付卡片Sandbox截图快照方案_[20260425-0321已采用].md` | 网站类完成卡片改为 sandbox 内真实运行后截图并持久化展示的计划方案，raw HTML 预览降级为 fallback |
